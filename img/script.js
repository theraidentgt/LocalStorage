window.onload = function()
{
		...
		for(var i = 0; i < listadoPersonas.length; i++)
		{
			...
			if(muestra)
			{
				...
				nom_div("e_" + i).addEventListener('click', function(event)
				{
					...
					nom_div('identifica').readOnly = true; // inhabilita la edición de el numero de documento ya que esta es el ID.  
					...
					Edita = true; id_persona = ind;
					("identifica").innerHTML = listadoPersonas[ind].identificacion
				});
				//Eliminar...
	...
	nom_div("guarda").addEventListener('click', function(event)
	{
		...
		if(correcto)
		{
			...
			{								
				var persona_editada = new persona(valores[0], valores[1], valores[2], valores[3], valores[4]);
				listadoPersonas[id_persona] = persona_editada;
				localStorage.setItem("listado", JSON.stringify(listadoPersonas));
				imprimeUsuarios();
				limpiarCampos();
				Edita = false;
